...
const ContentManager = () => {
  ...
  const fetchTechniques = async () => {
    setTechLoading(true);
    try {
      // --- แก้ไขตรงนี้: ระบุ relationship ให้ชัดเจนผ่าน practice_mode_id ---
      const { data, error } = await supabase
        .from('techniques')
        .select('*, practice_modes:practice_mode_id (name)') 
        .order('created_at', { ascending: false });
  ...
  const handleSaveMode = async () => {
    ...
    try {
      ...
      // 2. จัดการตารางลูก (practice_levels)
      // ใช้วิธีลบของเก่าทิ้งทั้งหมด แล้ว insert ใหม่ (Sync)
